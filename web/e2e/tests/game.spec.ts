...
test('seeded game: suit glyphs render, bidding works, a trick completes', async ({ page }) => {
  ...
  // No narration toggle in a regular game: narration is tutorial-only, so a "♪ on/off" control
  // here would mute a voice that never speaks on this screen (the toggle's label is the one ♪ the
  // accessibility mirror could carry — the suit glyphs above are ♦♠♥♣, never ♪).
