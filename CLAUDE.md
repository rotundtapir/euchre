...
The tutorial is narrated (0.2.0): one prerecorded clip per tutorial text id, under
`shared/src/commonMain/composeResources/files/narration/`, played through cardkit-ui's narration
plumbing — the same pipeline 500 uses.
...
(`{lessonId}-step-N` etc.) are the narration clip names — keep them stable and unique, and
re-record the clip whenever its text changes.
...
**Privacy invariant (since 0.2.0):** the FOSS build requests `INTERNET` for opt-in online play and
nothing else beyond the dynamic-receiver permission AndroidX adds; CI gates that allowlist.
Offline play still makes **no** network connections, and online is accountless — PRIVACY.md
promises exactly this, so a change to either must update it in the same commit. (v0.1.x requested
no `INTERNET` at all.)
...
- `docs/release-checklist.md` holds the one-time setup (keystore, Pages, GHCR, and the deferred
  AdMob / Play Console items) and the per-tag release gate — run the gate for every `v*` tag.
