...
#
# Regenerating a few clips: pass their ids. Use this both for a mangled take (generation samples, so
# one can come out wrong with nothing wrong in the text) and after editing a single narrated text:
#   scripts/generate-narration.sh basics-primer-1 bidding-step-2
# Only those clips are re-rendered, and only their manifest lines are rewritten. That is safe
# because the manifest is not the safety net — NarrationManifestTest is: any OTHER text you edited
# and forgot still fails the gate, since its recorded hash no longer matches its words.
set -euo pipefail
cd "$(dirname "$0")/.."
...
if [ "$#" -gt 0 ]; then
  ...
  SEND=$(mktemp)
  trap 'rm -f "$SEND"' EXIT
  TEXTS="$TEXTS" SEND="$SEND" python3 - "$@" <<'PYEOF'
import json, os, sys
wanted = sys.argv[1:]
...
with open(os.environ["SEND"], "w", encoding="utf-8") as out:
    for clip_id in wanted:
        ...
        if row is None:
            sys.exit(f"unknown clip id: {clip_id}")
...
digests = {r["id"]: hashlib.sha256(r["text"].encode()).hexdigest() for r in rows}
path = os.path.join(os.environ["OUT"], "manifest.txt")
header = ["# Generated by scripts/generate-narration.sh — do not edit.\n",
          f"# engine: qwen3-tts-1.7b-customvoice, voice: {os.environ['VOICE']}, "
          f"instruct: {os.environ['INSTRUCT']}\n"]
if only:
    # Patch in place: every other line keeps the hash recorded for the clip still on disk, so a text
    # edited but not regenerated stays visibly stale and NarrationManifestTest still catches it.
    kept = [l for l in open(path, encoding="utf-8") if not l.startswith("#") and l.strip()]
    lines = [(f"{l.split(' ', 1)[0]} {digests[l.split(' ', 1)[0]]}\n"
              if l.split(" ", 1)[0] in only else l) for l in kept]
else:
    lines = [f"{r['id']} {digests[r['id']]}\n" for r in rows]
